<script lang="ts">
	...
	export let solutionNumber: number;

	const toaster: Toaster = getContext("toaster");

	function share() {
		toaster.pop("Copied");
		navigator.clipboard.writeText(`${window.location.href}/${solutionNumber}`);
	}
</script>
...
<div class="share-historic" on:click={share} on:keypress={share}>
	...
	Copy link to this game ({modeData.modes[$mode].name} #{solutionNumber})
</div>
